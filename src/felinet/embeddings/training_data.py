...
def generate_questions_for_chunk(
    chunk_text: str, config: TrainingDataConfig, api_key: str
) -> list[str]:
    """
    ...
    """
    prompt = QUESTION_PROMPT.format(n=config.questions_per_chunk, passage=chunk_text)
...
def run_generation(chunks: list[dict], config: TrainingDataConfig | None = None) -> Path:
    """
